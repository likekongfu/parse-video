...
1. PDF、文档、图片或视频处理成功后，网页使用 `web_session` 调用 `POST /auth/processing-history`。
2. 用户 ID 只从 HttpOnly 网页会话解析，不接受客户端提交的 user_id。
3. 记录原文件名与大小、分类、功能类型、状态、输出文件名，以及可选的下载地址和过期时间。
4. 浏览器本地处理产生的 Blob 下载地址不持久化，只保存输出文件名；服务端转换结果可保存 HTTP(S) 或站内相对下载地址。
